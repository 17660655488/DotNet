...
namespace AURFIDReader
{
    public partial class AppForm : Form
    {
        ...
        internal string duplicate = null;
        internal Dictionary<string, string> inventory;
        internal string invtagList;
        internal List<string> tagTempList = new List<string>();
        internal string logName = "log_" + DateTime.Now.ToString("yyyy-MM-ddTHH-mm-ss") + ".txt";
        internal int logCount = 0;
        ...
        public AppForm() 
        {
            ...
            {   functionCallStatusLabel.Text = "Unable to Connect. Check Connections: "+e.ToString(); }
            try
            {
                inventory = System.IO.File.ReadAllLines(@"inventory.txt").Select(l =>
                {
                    var split = l.Split('	');
                    duplicate = split[0];
                    return new { Key = split[0], Value = split[3] };
                }).ToDictionary(p => p.Key, p => p.Value);
                //string dVal = "NA";
                //System.Windows.Forms.MessageBox.Show(inventory.TryGetValue("303402662C4E91C00000000C78687", out dVal).ToString());
            }
                    
            catch (Exception e)
            {
                System.Windows.Forms.MessageBox.Show(e.Message.ToString());
                functionCallStatusLabel.Text = duplicate;
                //functionCallStatusLabel.Text = "Inventry File Error: " + e.ToString();
            }
        }
                ...
                for (int nIndex = 0; nIndex < tagData.Length; nIndex++)
                {
                        ...
                        tagData[nIndex].OpStatus == ACCESS_OPERATION_STATUS.ACCESS_SUCCESS))
                    {
                        ...
                        if (isFound)
                        {
                            ...
                            try
                            {
                                count = uint.Parse(item.SubItems[1].Text) + tagData[nIndex].TagSeenCount; //uint.Parse(item.SubItems[2].Text) + tagData[nIndex].TagSeenCount;
                            ...
                            item.SubItems[1].Text = count.ToString();//tag.AntennaID.ToString();
                            item.SubItems[2].Text = DateTime.Now.ToString("yyyy-MM-ddTHH':'mm':'sszzz");//count.ToString();
                            if (inventory.ContainsKey(tag.TagID))
                            {
                                item.SubItems[3].Text = inventory[tag.TagID].ToString();// tag.PeakRSSI.ToString();
                            }
                            else{
                                item.SubItems[3].Text = "NotAvailable";
                            }
                        ...
                        else
                        {
                            ...
                            subItem = new ListViewItem.ListViewSubItem(item, tag.TagSeenCount.ToString());//tag.AntennaID.ToString());
                            item.SubItems.Add(subItem);

                            subItem = new ListViewItem.ListViewSubItem(item, DateTime.Now.ToString("yyyy-MM-ddTHH':'mm':'sszzz"));//tag.TagSeenCount.ToString());
                            m_TagTotalCount += tag.TagSeenCount;
                            item.SubItems.Add(subItem);

                            //invtagList += tag.TagID.ToString();
                            //if (m_TagTable.Count > 100)
                            //{
                            //    System.Windows.Forms.MessageBox.Show(invtagList.ToString());
                            //}
                            if (!tagTempList.Contains(tag.TagID))
                            {
                                tagTempList.Add(tag.TagID.ToString() + "   " + tag.MemoryBankData.ToString() + "   " + DateTime.Now.ToString("yyyy-MM-ddTHH':'mm':'sszzz"));
                            }
                            if (tagTempList.Count % 5 == 0)
                            {
                                logCount = tagTempList.Count;
                                System.IO.File.WriteAllLines(logName, tagTempList.ToArray());
                                //tagTempList = new List<string>();
                            }
                            
                            if (inventory.ContainsKey(tag.TagID))
                            {
                                subItem = new ListViewItem.ListViewSubItem(item, inventory[tag.TagID].ToString());
                            }
                            else
                            {
                                subItem = new ListViewItem.ListViewSubItem(item, "NotAvailable");
                            }
                            item.SubItems.Add(subItem);
            ...
            RunWorkerCompletedEventArgs connectEventArgs)
        {
            if (m_ConnectionForm.connectionButton.Text == "Connect")
            {
                if (connectEventArgs.Result.ToString() == "Connect Succeed")
                {
                    ...
                    this.connectionStatus.BackgroundImage =
                        global::AURFIDReaderNew.Properties.Resources.connected;
                }
            }
            else if (m_ConnectionForm.connectionButton.Text == "Disconnect")
            {
                if (connectEventArgs.Result.ToString() == "Disconnect Succeed")
                {
                    ...
                    this.connectionStatus.BackgroundImage =
                        global::AURFIDReaderNew.Properties.Resources.disconnected;
            ...
            try
            {
                if (m_IsConnected)
                {
                    if (readButton.Text == "Read")
                    {
                        ...
                        op.ReadAccessParams.MemoryBank = (MEMORY_BANK)2;
